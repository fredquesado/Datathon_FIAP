...
st.markdown('O projeto proporcionou diversos insights valiosos:)

st.markdown('• Influência Geopolítica nos Preços do Petróleo: O preço do barril de petróleo é fortemente influenciado pela geopolítica global. Isso é evidente nos anos de 1990-1991, durante a Guerra do Golfo, e novamente em 2008, com a crise financeira global. Esses eventos causaram flutuações significativas nos preços devido à incerteza e às mudanças na oferta e demanda')

st.markdown('• Relação com a Força do Dólar: Existe uma relação inversa entre o preço do petróleo e a força do dólar. Na análise inicial, observamos que quando o dólar se fortalece, os preços do petróleo tendem a cair.')

st.markdown('• Validade e Limitações dos Modelos Preditivos: Modelos de previsão de preços do petróleo, como o Random Forest Regressor utilizado neste projeto, são valiosos para embasar decisões estratégicas de curto e médio prazo. No entanto, a análise mostrou que a precisão do modelo diminui para previsões mais longas.')

st.markdown('• Impacto das Políticas de Energia Renovável: As políticas globais de transição para energias renováveis também têm um impacto significativo nos preços do petróleo.')
